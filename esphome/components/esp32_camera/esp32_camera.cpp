...
ESP32Camera::ESP32Camera() {
  ...
  // RHYX M21-45 modules can fail in native JPEG mode.
  // Capture RGB565 and convert to JPEG on demand.
...
ESP32CameraImage::ESP32CameraImage(camera_fb_t *buffer, uint8_t requesters, int brightness)
  ...
  if (this->buffer_->format == PIXFORMAT_JPEG) {
...
ESP32CameraImage::~ESP32CameraImage() {
  if (this->owns_jpeg_buffer_ && this->jpeg_buffer_ != nullptr) {
    free(this->jpeg_buffer_);
  }
}

camera_fb_t *ESP32CameraImage::get_raw_buffer() { return this->buffer_; }
uint8_t *ESP32CameraImage::get_data_buffer() { return this->jpeg_buffer_; }
size_t ESP32CameraImage::get_data_length() { return this->jpeg_length_; }
